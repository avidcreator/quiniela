...
1. An **external scheduler** (e.g. cron-job.org) hits `/api/cron/live` every
   minute, sending `Authorization: Bearer <CRON_SECRET>`.
...
> **Why an external scheduler, not Vercel Cron?** Vercel's **Hobby** plan only
> allows cron jobs that run **once per day**, which is useless for live scores.
> A free 1-minute scheduler that calls the secured endpoint works without
> upgrading to Vercel Pro. (If you do upgrade to Pro, re-add a `vercel.json`
> with `{"crons":[{"path":"/api/cron/live","schedule":"* * * * *"}]}` and drop
> the external one.)
...
2. Set env vars (locally in `.env.local`, and in **Vercel → Settings → Env**):
   ...
   - `CRON_SECRET` — any long random string (`openssl rand -base64 32`). The
     scheduler must send it as `Authorization: Bearer <CRON_SECRET>`; the
     endpoint rejects calls without it.
3. Deploy.
4. Set up the scheduler — e.g. at <https://cron-job.org> (free):
   - **URL:** `https://fifaquiniela.com/api/cron/live`
   - **Method:** GET
   - **Header:** `Authorization: Bearer <CRON_SECRET>`
   - **Schedule:** every 1 minute (the endpoint self-gates, so it's a cheap
     no-op outside match windows — see Notes).
5. In the admin portal → **En vivo**:
...
- API-Football rate limits apply per plan. To avoid burning quota around the
  ...
  for the live list + 1 per live match for events). Size your plan and the
  scheduler frequency for that in-match rate, not 24/7.
